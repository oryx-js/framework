...
export default class CoreDate extends Date {
    private static defaultTimezone: string = Common.env<string>('APP_TIMEZONE', 'UTC');
    private static locale: string = Intl.DateTimeFormat().resolvedOptions().locale;
    ...
    public format(fmt: string = 'Y-m-d H:i:s'): string {
        ...
        return fmt.replace(
            /[YymndjHhGgisAaDlMFU]/g,
            (match) => String(replacements[match] ?? match),
        );
    }

    public add(amount: number, unit: 'days' | 'hours' | 'minutes' | 'seconds' | 'weeks' | 'months' | 'years'): this {
        if (unit === 'days') {
    ...
    public subtract(amount: number, unit: 'days' | 'hours' | 'minutes' | 'seconds' | 'weeks' | 'months' | 'years'): this {
    ...
    public diff(date: CoreDate, unit: 'days' | 'hours' | 'minutes' | 'seconds' | 'weeks' | 'months' | 'years'): number {
